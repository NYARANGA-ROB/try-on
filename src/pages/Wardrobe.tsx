...
const Wardrobe: React.FC = () => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!apiKey) {
      setUploadError('No API key configured. Please set up your OpenAI API key first.');
      return;
    }

    console.log('API Key validation check...');
    try {
      const { testApiKey } = await import('../services/openai');
      ...
      setUploadError('Error validating API key. Please check your connection.');
      return;
    }
    ...
    try {
      ...
      let packshotImage: string | undefined;
      try {
        packshotImage = await extractPackshot(apiKey!, selectedFile, itemName, quality);
        console.log('Packshot image generated');
      } catch (packshotError) {
        console.warn('Packshot generation failed, continuing with original image:', packshotError);
        // Continue without packshot - user can regenerate later
      }
      ...
      setUploadError(`Error uploading item: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  return (
    ...
    >
      ...
      {/* Debug components - remove after fixing the issue */}
      <ApiKeyDebug />
      <DatabaseDebug />
