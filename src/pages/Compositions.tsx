...
export default function Compositions() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(`Error generating try-on: ${error instanceof Error ? error.message : 'Unknown error'}`);
      
      // Local fallback: save composition using the selected pose image
      try {
        const pose = profilePhotos?.find((p) => p.id === selectedProfileId);
        if (!pose) throw new Error('Pose not found');

        // pick the first selected wardrobe item id across categories
        const firstSelectedId = (Object.values(selectedByCat).find((v) => v != null) as number | null | undefined) ?? 0;

        await addComposition({
          name: compositionName + ' (placeholder)',
          profilePhotoId: selectedProfileId,
          wardrobeItemId: firstSelectedId,
          resultImage: `data:image/jpeg;base64,${pose.image}`
        });
        ...
        setError('Error saving composition. Please try again.');
      }
    } finally {
      setIsGenerating(false);
    }
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {profilePhotos?.map((p) => (
                      <button
                        key={p.id}
                        type="button"
                        onClick={() => setSelectedProfileId(p.id ?? null)}
            ...
            {compositions?.map((c) => (
              ...
              >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setSelectedComposition(c.id ?? null);
